...
ulong evSymbolCount;
ulong copiedSymbols = 0;

char **strtabByIndex;
char *strtabBuffer;

struct KernelSymbol **evHash;
ulong segmentCopyBuffer;
...
{
	ulong bucketIndex = HashString(sym->name) % evSymbolCount;
...
{
	ulong bucket = HashString(name) % evSymbolCount;
...
void InitLoader(struct KernelModule *ldrObj)
{
	...
	ldrObj->segments = (struct ProgramHeader *) (ldrObj->fileBase
			+ ldrMap->programHeaderOffset);
...
void CopySegments(struct KernelModule *ldrObj)
{
	...
	ulong farthestAddress = 0, segLimit;
...
void FindRelocationTables(struct KernelModule *ldrObj)
{
	...
	ldrObj->linkerTable = (struct DynamicEntry*) (ldrObj->realBase
			+ dynSeg->virtualAddress);
	...
	if (relocTable != null) {
		...
		ldrObj->pltrel.entryCount = relocTotalSize->val
				/ ldrObj->pltrel.entrySize;
	}
}
...
void CopySymbols(struct KernelModule *ldrObj)
{
	...
	ulong symCount =
			((ulong*) (ldrObj->realBase + hashTab->ptr))[1];
...
void InitRelocator(struct KernelModule *envObjectCache,
		struct KernelSymbol *envSymbolTable, ulong envSymbolCount,
		ulong _moduleCount, ulong segmentCopy)
{
	...
	strtabByIndex = (char **) ((ulong) envSymbolTable
			+ envSymbolCount
					* (sizeof(struct KernelSymbol)
							+ sizeof(struct KernelSymbol*)) + 128 /*a buffer*/);
...
void RelocateModule(struct KernelModule *ldrObj)
{
	...
	if (ldrObj->pltrel.relocType == DT_REL)
		ResolveAllRel(ldrObj->pltrel.relEntries, ldrObj,
				ldrObj->pltrel.entryCount);
	else
		ResolveAllRela(ldrObj->pltrel.relaEntries, ldrObj,
				ldrObj->pltrel.entryCount);
}
