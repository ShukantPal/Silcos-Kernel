...
bool IsElfFile(void *file)
{
	...
	if (fileAlias->fileIdentifier[EI_MAG0] != ELFMAG0)
		return (false);
	if (fileAlias->fileIdentifier[EI_MAG1] != ELFMAG1)
		return (false);
	if (fileAlias->fileIdentifier[EI_MAG2] != ELFMAG2)
		return (false);
	if (fileAlias->fileIdentifier[EI_MAG3] != ELFMAG3)
		return (false);
...
{
	struct ProgramHeader *segmentEntry =
			(struct ProgramHeader *) ((unsigned long) fileHdr
					+ fileHdr->programHeaderOffset);
	unsigned long segEntryCount = fileHdr->programHeaderEntryCount;

	for (unsigned long index = 0; index < segEntryCount;
			index++, segmentEntry++) {
	...
	if (segmentEntry->entryType != PT_DYNAMIC) {
		WriteLine(
				"Warning: Invalid ELF-object found: No dynamic segment present!");
		WriteLine("Warning: Assuming that the ELF-object contains no symbols!");
		*symbolCount = 0;
		*stringSize = 0;
	}

	struct DynamicEntry *dynEnts =
			(struct DynamicEntry *) ((unsigned long) fileHdr
					+ segmentEntry->fileOffset);
