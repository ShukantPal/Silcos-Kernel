...
/* This is the C++ namespace :-) */

static const char *nsRootLocalName = "";
static const char *nsRootGlobalName = "::";
static bool initEnsured = false;

/*
 * This object is used during module initialization and
 * before all global objects are constructed. Hence, use
 * ensureInit() before using it (for early-usage)
 */
::Module::Namespace Namespace::nsRoot;
...
Namespace::Namespace(const char *name, Namespace *owner)
{
	...
	unsigned long pgIdentSize =
			(owner != null) ? strlen(owner->nsGlobalIdentifier) + 2 : 2;
...
Namespace::Namespace(const char *fromFile, const char *name, Namespace *owner)
{
	unsigned long lIdentSize = strlen(name);
	unsigned long pgIdentSize =
			(owner != null) ? strlen(owner->nsGlobalIdentifier) : 0;
...
void Namespace::ensureInit()
{
	if(initEnsured)
		return;
	nsRoot.fileName = static_cast<char *>(null);
	nsRoot.nsLocalIdentifier = const_cast<char *>(nsRootLocalName);
	nsRoot.nsGlobalIdentifier = const_cast<char *>(nsRootGlobalName);
	nsRoot.nsOwner = static_cast<Namespace *>(null);
	nsRoot.totalChildren = 0;
	initEnsured = true;
}
 ...
 */
Namespace *Namespace::searchOnly(const char *dChildIdentBuffer, unsigned bytes)
{
	for (Namespace *dChild = static_cast<Namespace *>(nsChildSet.head);
			dChild != null; dChild = static_cast<Namespace *>(dChild->next)) {
		if (strcmpn(dChildIdentBuffer, dChild->nsLocalIdentifier, bytes))
			return (dChild);
	}
...
Namespace *Namespace::searchRelative(const char *dChildIdentBuffer)
{
	...
	return ((nextInPath != null) ?
			nextInPath->searchRelative(dChildIdentBuffer + localIdentSize + 2) :
			null);
}
