 ...
 * @file Main.cpp
 *
 * Implements the kernel boot protocol by a small set of functions
 * that initialize each subsystem and then passes control to the
 ...
 */
void ImportLinkerRaw(ulong physEntriesAddress)
{
	KernelModule *moduleEnt = (KernelModule *) (physEntriesAddress
			+ KERNEL_OFFSET - kernelParams.loadAddress);
	MultibootTagModule *btldrInfo = MultibootChannel::getFirstModule();
	ModuleContainer *modObj;
	Module::Elf::DynamicEntry *ltable, *ent;

	kernelParams.bootModules = 0;
	Namespace::ensureInit();
	...
	while (btldrInfo != null) {
		...
		modObj->setPhys(moduleEnt->realBase);

		ltable = (Module::Elf::DynamicEntry *) ((ulong) moduleEnt->linkerTable
				+ KERNEL_OFFSET - kernelParams.loadAddress);
...
//			modObj->init();// TODO: Fix bug causing crash here!
		...
		if (modObj->initFunctorCount)
			Arrays::invokeAll(modObj->initArray, modObj->initFunctorCount);
 ...
 * @param totalSymbols
 */
void ImportAllSymbols(ulong physSymbolTable, ulong totalSymbols)
{
	KernelSymbol *symEnt = (KernelSymbol *) (physSymbolTable + KERNEL_OFFSET
			- kernelParams.loadAddress);

	for (uint symidx = 0; symidx < totalSymbols; symidx++, symEnt++) {
 ...
 *
 * @param retAddr address from where execution should continue after
 * 		initialization of pagination.
 * @param envBase physical base of the kernel environment
 * @param envSize size of the kernel environment
 * @see Entry.S for an example usage
 */
export_asm void EarlyMain(ulong retAddr, ulong envBase, ulong envSize)
{
	ulong autodat = envBase + envSize - AUTO_DAT;

	U64 *globalTable = (U64*) (autodat);
	U64 *globalDirectory = (U64*) (autodat + PAGE);
	U64 *identityDirectory = (U64*) (autodat + 2 * PAGE);
	U64 *bootPDPT = (U64*) (autodat + 3 * PAGE);

	memsetf(bootPDPT, 0, 32);
	memsetf(identityDirectory, 0, PAGE);
	memsetf(globalDirectory, 0, PAGE);
	memsetf(globalTable, 0, PAGE);

	Pager::init(retAddr, envBase, globalTable, globalDirectory,
			identityDirectory, bootPDPT);
}
 ...
 */
export_asm void Main(ulong bootTable, ulong bootKernelBase,
		ulong bootKernelSize, ulong globalSymbols, ulong totalSymbols,
		ulong kernelStrings, void *globalSymbolHash, ulong pmoduleEntries)
{
	InitConsole(reinterpret_cast<U8*>(CONSOLE));
	DbgLine("Reporting Control: \"KernelHost\" @Silcos.KernHost.3050");

	boot_base = bootKernelBase;
	ulong autodat = KERNEL_OFFSET + bootKernelSize - AUTO_DAT;
	Pager::init2((U64 *) autodat + 512, (U64 *) autodat,
			bootKernelBase + bootKernelSize - AUTO_DAT + PAGE * 3);
	MultibootChannel::init(bootTable);

	kernelParams.loadAddress = bootKernelBase;
	kernelParams.loadSize = bootKernelSize;
	kernelParams.pageframeEntries = getHugePagesSize(
			bootKernelBase + bootKernelSize);
