...
import_asm void SetupPageProtection(ulong jmpAddr, U64 *bootPDPT);
 ...
 * @param bootPDPT
 */
void Pager::init(unsigned long retAddr, unsigned long envBase, U64 *globalTable,
		U64 *globalDirectory, U64 *identityDirectory, U64 *bootPDPT)
{
	globalTable[0] = (0xB8000) | 3;/* Map CONSOLE to the video RAM page */
	globalTable[511] = ((U32) globalDirectory) | 3;

	identityDirectory[PageExplorer::getDirectoryIndex(envBase)] = (envBase | 3
			| (1 << 7));

	globalDirectory[0] = (U32) (envBase & ~0x1FFFFF) | 3 | (1 << 7);
	globalDirectory[1] = ((U32) (envBase & ~0x1FFFFF) + 0x200000) | 3
			| (1 << 7);

	globalDirectory[510] = ((U32) globalDirectory) | 3;
	globalDirectory[511] = ((U32) globalTable) | 3;

	bootPDPT[0] = ((U32) identityDirectory) | 1;
...
{
	extern int boot_base;
	Pager::globalDirectory = globalDirectory;
	Pager::globalTable = globalTable;
	globalDirectory[PageExplorer::getDirectoryIndex(boot_base)] = 0;
		...
		PageAttributes pgAttr)
{
	...
	if (pgTable != NULL) {
		pgTable[PageExplorer::getTableIndex(vadr)] = (padr & 0xFFFFF000)
				| pgAttr | 3;
		FlushTLB(vadr);
	}
}
		...
		PageAttributes attr)
{
	...
	if (pgDir != null) {
		pgDir[PageExplorer::getDirectoryIndex(vadr)] = (padr & ~0x1FFFFF) | attr
				| (1 << 7);
		FlushTLB(vadr);
	}
}
 ...
 * @author Shukant Pal
 */
void Pager::mapAll(VirtAddr vaddr, PhysAddr paddr, unsigned mapSize,
		unsigned allocFlags, PageAttributes pgAttr)
{
		...
		PageAttributes attr)
{
	U64 *pte = PageExplorer::getAllPageTables(base >> 21,
			getPageTableScope(limit - base + (base % KPGSIZE)), allocFlags)
			+ ((base & 0x1FFFFF) >> 12);
 ...
 * @author Shukant Pal
 */
void Pager::useAllHuge(VirtAddr base, VirtAddr limit, unsigned allocFlags,
		PageAttributes attr)
{
	if (base & 0x1FFFFF)
 ...
 * @author Shukant Pal
 */
void Pager::use(VirtAddr vadr, unsigned allocFlags, PageAttributes pgAttr)
{
 ...
 * @author Shukant Pal
 */
void Pager::useAll(VirtAddr vBase, VirtAddr vLimit, unsigned allocFlags,
		PageAttributes attr)
{
