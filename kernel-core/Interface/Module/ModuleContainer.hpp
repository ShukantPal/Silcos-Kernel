...
public:
	...
	ulong getBase(){ return (baseAddress); }
	DynamicLink *getLinker(){ return (linkerInfo); }
	ulong getPhys(){ return physicalAddress; }
	const char *ident(){ return (buildName); }
	void init(){ initFunctor(); }
	void fini(){ finiFunctor(); }
	ulong load(){ return (physicalAddress); }
	void setBase(ulong base){ baseAddress = base; }
	void setLinker(DynamicLink *nli){ linkerInfo = nli; }
	void setPhys(ulong phys){ physicalAddress = phys; }

	ModuleContainer(const char *buildName, ulong buildVersion);
protected:
	~ModuleContainer();
private:
