 ...
 * @file Math.hpp
 *
 * Provides numerous math utilities for comparison, arithmetic, and
...
public:
	...
	static inline void bitSet(unsigned char &n, unsigned long idx)
	{
		n |= (1 << idx);
	}

	static inline void bitSet(unsigned short &n, unsigned long idx)
	{
		n |= (1 << idx);
	}

	static inline void bitSet(unsigned int &n, unsigned long idx)
	{
		n |= (1 << idx);
	}

	static inline void bitSet(unsigned long &n, unsigned long idx)
	{
		n |= (1 << idx);
	}
	...
	static inline bool bitTest(unsigned int n, unsigned long idx)
	{
		return ((bool)(n >> idx));
	}
