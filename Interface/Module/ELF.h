...
#define PROGRAM_HEADER(eHeader) ((struct ElfProgramHeader *) ((ADDRESS) eHeader + eHeader->ProgramHeaderOffset))
#define SECTION_HEADER(eHeader) (eHeader->SectionHeaderOffset) ? ((struct ElfSectionHeader *) ((ADDRESS) eHeader + eHeader->SectionHeaderOffset))
...
enum DynamicTag {
	...
	DT_FINI			=	13,// Element holds the address of the initialization function
...
struct ElfRelaTable {
	struct ElfRelaEntry *EntryTable;
	ULONG EntryCount;
};
...
struct ElfProgramCache {
	ULONG PageCount;/* Total number of pages */
	struct ElfProgramHeader *Dynamic;
	struct ElfDynamicTable DynamicTable;
};
...
typedef
struct ElfCache {
	struct ElfSymbolTable dsmTable;/* Dynamic-symbol table */
	struct ElfHashTable dsmHash;/* Dynamic-symbol hash-table (normal hash-table, not GNU-Hash) */
	struct {	
		struct ElfRelaEntry *eRelaTable;
		SIZE eRelaCount;
	};
	struct {
		struct ElfRelEntry *eRelTable;
		SIZE eRelCount;
	};

	struct ElfProgramCache ProgramHdrCache;
	Void *ePLT;
	Void *eGOT;
