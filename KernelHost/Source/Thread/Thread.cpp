...
void InitTTable(void){
	...
	tdInfo = KiCreateType(tdName, sizeof(Thread), sizeof(unsigned long), &KiThreadConstruct, NULL);
	kIdlerThread = (Thread*) KNew(tdInfo, KM_NOSLEEP);
	kInitThread = (Thread*) KNew(tdInfo, KM_NOSLEEP);

	/* Initialize Fields */
	Thread *kInit = kInitThread;
	KiSetupRunnable((KTask*) kInit, (ADDRESS) kInit, ThreadTp);
...
void SetupRunqueue()
{
	...
	KiSetupRunnable((KTask*) idlerThread, (ADDRESS) idlerThread, ThreadTp);
		
	idlerThread->Gate.RRM_ID = 100;
		
	KiSetupRunnable((KTask*) setupThread, (ADDRESS) setupThread, ThreadTp);
