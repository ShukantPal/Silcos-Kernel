...
bool kfree(void* memGiven, bool forceDelete)
{
	...
	else
	{
		...
		MMFRAME *paddr = GetFrames((ADDRESS) memGiven,
							vmBlockOrder, KERNEL_CONTEXT);
		KeFrameFree((PhysAddr) paddr);
		Pager::disposeAll((unsigned long) memGiven,
				(unsigned long) memGiven + (1 << (vmBlockOrder + KPGOFFSET)));
